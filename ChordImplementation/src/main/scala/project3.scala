...
case class Route1(Key: BigInt, numOfHops: Int, Sourcenode: BigInt, Path: String)
...
class ChordActor(numNodes: Int, numRequests: Int, nodeId: BigInt, m: Int) extends Actor {
  ...
  def receive = {
    case generateRequest(key) =>
      ...
      var (closestMatch, closestMatchId) = findClosestPrecedingId(key, self, nodeID, "Route1").asInstanceOf[(ActorRef, BigInt)]

      if (closestMatch == self) {
        tracker ! RoutingFinished(nodeID, numOfHops, nodeID, path, key, predecessorId)
      } else {
        closestMatch ! Route1(key, numOfHops, nodeID, path)
      }
    ...
    case Route1(key, numOfHops, sourcenode, path) =>
      ...
      if (closestMatch == self) {
        ...
        closestMatch ! Route1(key, hops, nodeID, newpath)
      }
